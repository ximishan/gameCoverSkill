...
Do not rely on a generative image model to satisfy an exact 35% coordinate. Direct image-generation typography is only acceptable when the user explicitly asks for all text to be generated inside the image; in that case, placement can only be approximate and must not be described as pixel-exact.
...
- deterministic text rendering is used whenever exact 35% placement is required
